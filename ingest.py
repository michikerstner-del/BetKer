...
def get_or_create_id(cur, table, name):
    # Wenn kein Name da ist, gib None zurück
    if not name or name == "TBD":
        return None
    
    # 1. Schauen, ob das Team/Stadion schon existiert
    cur.execute(f"SELECT id FROM public.{table} WHERE name = %s", (name,))
    result = cur.fetchone()
    if result:
        return result[0]
    
    # 2. Wenn nicht, neu anlegen und die neue ID zurückgeben
